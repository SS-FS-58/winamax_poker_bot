...
class MonteCarlo(object):
    ...
    def distribute_cards_to_players(self, deck, player_amount, player_card_list, known_table_cards,
        ...
        for player_cards in player_card_list:
            ...
            if type(player_cards) == set:
                ...
                if random_card2 > random_card1:
                    player_cards.append(deck.pop(random_card2 - 1))
                else:
                    player_cards.append(deck.pop(random_card2))
        ...
        for _ in range(player_amount - knownPlayers):
            ...
            if random_card2 > random_card1:
                random_player.append(deck.pop(random_card2 - 1))
            else:
    ...
    def run_montecarlo(self, logger, original_player_card_list, original_table_card_list, player_amount, ui, maxRuns,
        ...
        if ghost_cards != '':
            for card in ghost_cards:
                try:
                    OriginalDeck.pop(OriginalDeck.index(card))
                except:
                    pass
        ...
        for m in range(maxRuns):
            ...
            if winner < CollusionPlayers + 1:
                ...
                defeatCardTypeList.append(winnerCardType)

            self.equity = np.round(wins / runs, 3)

            if runs > 999 and time.time() > timeout:
        ...
        self.defeatCardTypeList = Counter(defeatCardTypeList)
        for key, value in self.defeatCardTypeList.items():
            self.defeatCardTypeList[key] = value / runs

        self.defeatTypesDict = self.defeatCardTypeList.items()
        self.runs = runs
        self.passes = passes

        return self.equity, self.winTypesDict, self.defeatTypesDict
...
def run_montecarlo_wrapper(p, ui_action_and_signals, config, ui, t, L, preflop_state, h):
    ...
    if p.selected_strategy['collusion'] == 1 and (t.gameStage != "PreFlop" or h.round_number != 0):
        collusion_cards, collusion_player_dropped_out = t.get_collusion_cards()
    ...
    #     collusion_cards, collusion_player_dropped_out = L.get_collusion_cards(h.game_table_name, t.gameStage)
    ...
    if t.gameStage != 'PreFlop':
        try:
            ...
            # Limpin card set
            opponent_range_level = round(7 + random.uniform(0, 1)*2)
            ranges = set()
            for i in range(opponent_range_level):
                ranges.update(tier[i])
            range_pecent = 30
            opponent_range = round(range_pecent + random.uniform(0, 1)*(100 - range_pecent))
    ...
    if t.gameStage != 'PreFlop' and p.selected_strategy['use_relative_equity']:
        if p.selected_strategy['preflop_override'] and preflop_state.preflop_bot_ranges!= None:
            ...
            t.range_equity, _, _ = m.run_montecarlo(logger, t.player_card_range_list_and_others, t.cardsOnTable,
    ...
    # if t.gameStage != "PreFlop":
    t.abs_equity, _, _ = m.run_montecarlo(logger, t.PlayerCardList_and_others, t.cardsOnTable, int(t.assumedPlayers), ui, maxRuns=maxRuns,
                    ghost_cards=ghost_cards, timeout=timeout, opponent_range=opponent_range)
    
    # set table model equity

    t.table_model_data['equity'] = t.abs_equity
    ...
    if len(t.mycards) != 2:
        return m
    if t.gameStage == "PreFlop" and len(t.mycards) > 1:
